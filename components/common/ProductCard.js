...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="group relative bg-zinc-900 border border-zinc-800 rounded-lg shadow-sm transition-all duration-300 overflow-hidden hover:shadow-[0_0_20px_rgba(255,255,255,0.15)]">

      {/* Badge container */}
      <div className="absolute top-0 left-0 w-full p-2 md:p-3 flex justify-between items-start z-20 pointer-events-none">
        {badge ? (
          <span className="bg-white text-black text-[9px] md:text-[10px] font-bold px-2 py-0.5 md:px-3 md:py-1 uppercase tracking-widest pointer-events-auto">
            {badge}
          </span>
        ) : <div></div>}
      ...
      <Link href={`/product/${id}`} className="relative block h-[220px] md:h-[320px] overflow-hidden bg-[#111]">
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out z-10 hidden md:block">
          <button className="w-full py-3 bg-white/95 backdrop-blur-md text-black text-sm font-bold uppercase tracking-wider hover:bg-white transition-colors">
            Quick View
          </button>
        </div>
      </Link>
      ...
      {/* Content */}
      <div className="p-3 md:p-5 flex flex-col gap-2 md:gap-3 relative bg-zinc-900 border border-zinc-800">
        <div>
          {category && (
            <p className="text-[9px] md:text-[10px] text-white font-bold uppercase tracking-widest mb-1">
              {category}
            </p>
          )}
          <Link href={`/product/${id}`}>
            <h3 className="text-white text-base md:text-lg font-medium leading-snug hover:text-white transition-colors line-clamp-1">
              {name}
            </h3>
          </Link>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <IconStar size={14} className="fill-[var(--primary)] text-white" />
        ...
        <button className="w-full mt-2 py-2 md:py-3 border border-white/20 text-white text-xs md:text-sm font-semibold uppercase tracking-wider hover:border-white hover:bg-[var(--white)] hover:text-black transition-all">
          Add to Cart
        </button>
      </div>
    </div>
